...
import { createDocNode } from "./support.data";
import { DocNodeType } from "./types";
...
describe("routes", () => {
  ...
  test("Create and read generic data", async () => {
    ...
    expect(result.data.text).toEqual(data.text);
    expect(result.data.type).toEqual(data.type);
    expect(result.data.links.length).toEqual(0);
  });
