...
describe('routes', () => {
  ...
  test('CRUD public owner-less perspectives', async (done) => {
    ...
    expect(result1.data.object.payload.remote).toEqual(LOCAL_EVEES_PROVIDER)

    /** update head */
    const message = 'commit message'

    let text1 = 'new content'
    let par1Id = await createData(
      { text: text1, type: DocNodeType.paragraph, links: [] },
      '',
    )
    let commit1Id = await createCommit(
      [creatorId],
      45678,
      message,
      [],
    ...
    let par2Id = await createData(
      { text: text2, type: DocNodeType.paragraph, links: [] },
      '',
    )
    let commit2Id = await createCommit(
      [creatorId],
      984321,
      message,
      [commit1Id],
  ...
  test('CRUD private perspectives', async (done) => {
    ...
    expect(result2.data.object.payload.remote).toEqual(LOCAL_EVEES_PROVIDER)
    ...
    let par3Id = await createData(
      { text: text3, type: DocNodeType.paragraph, links: [] },
      user1.jwt,
    )
    let commit3Id = await createCommit(
      [creatorId],
      8787123,
      message,
      [commit2Id],
  ...
  test('CRUD private perspective inherited', async (done) => {
    ...
    expect(result5.data.object.payload.remote).toEqual(LOCAL_EVEES_PROVIDER)
    ...
    expect(result6.data.object.payload.remote).toEqual(LOCAL_EVEES_PROVIDER)
